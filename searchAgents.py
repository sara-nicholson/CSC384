...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
	goalState = all(i in state[1] for i in self.corners)
	return goalState
...
def cornersHeuristic(state, problem):
	...
	if corners[i] not in state[1]:
	    ...
	    distx = (x-corners[i][0])
	    disty = (y-corners[i][1])
	    #make the path that has more corners visited a lower cost
	    cornersvisited = 0
   	    if (len(state[1])!=0):
	        wallsvisted = len(corners) - len(state[1])		
	    mindist = abs(distx)+abs(disty)-cornersvisited + wallsdiff
    hcost.append(mindist)
...
def foodHeuristic(state, problem):
    ...
    if foodGrid.count() == 0:
	return 0
    #if the successorhas food return 1
    if state[0] in foodlist:
	return 1 
    #otherwise find the path to the closest food and return it	
    else:  

   	closestfood = ()
	foodx,foody = foodlist[0]
	diffx = abs(foodx-x)
	diffy = abs(foody-y)
	closestdistance = diffx+diffy
	for i in range(len(foodlist)):
	    foodx,foody = foodlist[i]
 	    diffx = abs(foodx-x)
	    diffy = abs(foody-y)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        icomplete the problem definition.
        """
        x,y = state
